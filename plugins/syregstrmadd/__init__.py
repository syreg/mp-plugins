...
class SyregStrmadd(_PluginBase):
    ...
    plugin_author = "syreg"
    # 作者主页
    author_url = "https://github.com/syreg"
    # 插件配置项ID前缀
    plugin_config_prefix = "syregstrm_"
    # 加载顺序
    plugin_order = 100
    # 可使用的用户级别
    auth_level = 1
    ...
    def scan(self, event: Event = None):
        ...
        if event:
            event_data = event.event_data
            if not event_data or event_data.get("action") != "syreg_strm_add":
                return
                        ...
                        if not SystemUtils.exits_files(Path(increment_file).parent, []):
                            ...
                            for parent_path in Path(increment_file).parents:
                                if parent_path.name in self._no_del_dirs:
                                    break
                                if len(str(parent_path)) <= len(str(increment_dir)):
                                    break
                                else:
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'text': '目录监控格式：'
                                                    ...
                                                    '2.增量目录#监控目录#目的目录#cd2#cd2挂载本地根路径#cd2服务地址；'
                                                    '3.增量目录#监控目录#目的目录#alist#alist挂载本地根路径#alist服务地址。'
                                        }
                                    }
                                ]
                            }
